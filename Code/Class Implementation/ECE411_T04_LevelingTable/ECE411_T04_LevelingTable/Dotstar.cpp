...
void CDotstar::show(void)
{
	...
	for (int i = 0; i < 4; ++i)	
		{ spiOut(START_FRAME); } // start-frame marker
	...
	do {
		...
		for (int j = 0; j < NUM_SUBPIXELS; ++j) 
		{	
			spiOut(mPixels[pixelCounter][j]);  // RGB
		}
	} while(--pixelCounter);
	
	// send finished command
	for (int i = 0; i < 4; i++)
		{ spiOut(END_FRAME); } // End-frame marker 
}
...
//*************************************************
uint8_t CDotstar::getPixels(uint8_t pixel, uint8_t subpixel)
{
	return mPixels[pixel][subpixel];
}
