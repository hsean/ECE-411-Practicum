...
void setPixelColor(struct Dotstar* strip,   // set color of single pixel
			       uint16_t pixelPos,
	               uint32_t color);
void setPixelColor(struct Dotstar* strip,   // set color by subpixel
	               uint16_t pixelPos,
	               uint8_t red,
	               uint8_t green,
	               uint8_t blue);
...
//*************************************************
void setPixelColor(struct Dotstar* strip, uint16_t pixelPos,
	uint8_t red, uint8_t green, uint8_t blue)
{
	// check that position is not out of bounds
	if (pixelPos < strip->mNumPixels)
	{
		strip->mPixels[pixelPos][RED_SUBPIXEL] = red;
		strip->mPixels[pixelPos][GREEN_SUBPIXEL] = green;
		strip->mPixels[pixelPos][BLUE_SUBPIXEL] = blue;
	}
}
...
void show(struct Dotstar* strip)
{
	...
	for (int i = 0; i < 4; ++i)
	{
		spiOut(strip,START_FRAME);
	} // start-frame marker

	// send color for each subpixel
	do {
		spiOut(strip, PIXEL_START);    // pixel start
		for (int j = 0; j < NUM_SUBPIXELS; ++j)
		{
			spiOut(strip->mPixels[pixelCounter][j]);  // RGB
		}
	} while (--pixelCounter);
...
{
	// set data pin low
	pinOutput(DATA_PIN, LOW);

	// set clock pin low
	pinOutput(CLOCK_PIN, LOW);
}
...
uint8_t getPort(uint8_t pinNumber)
{
	...
	case 11: return PORTD;
	case 12: return PORTD;
	case 13: return PORTD;
	case 14: return PORTB;
	case 15: return PORTB;
	case 16: return PORTB;
	case 17: return PORTB;
	case 18: return PORTB;
	case 19: return PORTB;
	case 23: return PORTC;
	case 24: return PORTC;
	case 25: return PORTC;
	case 26: return PORTC;
	case 27: return PORTC;
	case 28: return PORTC;
	default: return ERROR;  // pin not found
	}
}
