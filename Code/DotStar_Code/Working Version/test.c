...
//definitions
#define MIN_ANGLE 50                                //minimum servo angle
#define MAX_ANGLE 120                               //maximum servo angle
#define PWM_MIN 1000                                 //minimum servo pulse width modulation value
#define PWM_MAX 1900                               //maximum servo pulse width modulation value
...
#define ACCL_NUM_SAMPLE 100                          //number of times to sample the accelerometer
...
#define SERVO_MV_DELAY_MS 500                         //delay to allow servos to move
#define M_PI 3.14159265358979323846                 //pi
#define INI_COLOR 0xFF00FF
...
int main()
{
	...
	while(1) {
		...
		for(uint32_t j = 0; j < 0xFF; ++j)
		{
			for(int i = 0; i < NUM_PIXELS; ++i)
			{
				mid = j << 8;
				high = j << 16;
				uint32_t color = j | mid | high;
				
				setPixelColor(strip, i, color);
			}
			show(strip);
			_delay_ms(50);
		}
	...
	//temporary stuff
	DDRD |= (1<<DDD4)|(1<<DDD5)|(1<<DDD6)|(1<<DDD7);          // set LED pin PD1 to output
	...
	while(1)
	{
		...
		/*for(i = 0; i < 100; i += 10)
		{
			...
			_delay_ms(10);     //servo movement delay
		}
...
void servoFix(int x, int y, float table[])
{
	...
	s0 = table[home] + yDist;
	s1 = table[home] + xDist - yDist;
	s2 = table[home] - xDist - yDist;
...
void PositionSetup(float heightMap[])
{
	...
	for (degree = 270; degree <= 380; ++degree)
	{
